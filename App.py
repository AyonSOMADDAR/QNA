...
def get_pdf_text_from_backend(pdf_paths):
    """Extract text from PDF files provided by backend (server-side)."""
    text_pages = []
    for pdf_path in pdf_paths:
        try:
            ...
            st.error(f"Error reading PDF file {pdf_path}. Check logs for details.")
    return text_pages
...
def main():
    ...
    st.set_page_config(page_title="FAQ H2H")
    logo_url = 'image.png'
    st.image(logo_url,width= 500)
    
    st.header("Hack 2 Hire FAQ")
    ...
    with st.sidebar:
        ...
        pdf_paths = ["data.pdf", ]
        if pdf_paths:
            text_pages = get_pdf_text_from_backend(pdf_paths)
            if text_pages:
                text_chunks = get_text_chunks(text_pages)
                if text_chunks:
                    get_vector_store(text_chunks)
                    st.success("Feel Free to ask Any Doubts")
            else:
